...
const gods={甲:'比肩',乙:'劫财',丙:'食神',丁:'伤官',戊:'偏财',己:'正财',庚:'七杀',辛:'正官',壬:'偏印',癸:'正印'};
const vectors={甲:[1,-2,0,2],乙:[0,-3,-1,3],丙:[4,3,1,2],丁:[5,2,-1,1],戊:[2,6,2,2],己:[2,5,4,1],庚:[3,0,-2,0],辛:[5,1,1,2],壬:[3,0,0,5],癸:[3,1,1,5]};
const profiles={
 support:{label:'火土调候',description:'寒湿偏重时，火、土出现额外加分；水、木相应减分，金轻度减分。',prefs:{金:-1,水:-5,木:-4,火:6,土:5}},
 balanced:{label:'中性参考',description:'不预设哪一种五行更有帮助，只计算十神与干支关系。',prefs:{金:0,水:0,木:0,火:0,土:0}},
 release:{label:'水木顺势',description:'按从印顺势口径，水、木出现额外加分；火、土、金相应减分。',prefs:{金:-3,水:5,木:4,火:-4,土:-5}}
};
const weights=[.3,.3,.2,.2],keys=['career','wealth','relationship','opportunity'],names=['事业','财运','感情','贵人'];
const natal=['壬午','壬子','甲寅','丙寅'],positions=['年柱','月柱','日柱','时柱'];
...
function factors(annual,dayun,profile){
 ...
 if('丑未'.includes(annual[1]))add('G-天乙','甲日天乙贵人取丑、未：流年'+annual[1]+'命中；仅加贵人项',[0,0,0,4]);
 return rows;
}
...
function exportData(calendar,profile){return {schemaVersion:'life-terminal/1.0',modelVersion:'rules-1.1.0-jia',profile,profileLabel:profiles[profile].label,birth:{gender:'女',civil:'2002-12-12 北京时间 04:00–05:00（建模取中点 04:30）',place:'宁夏固原市 · 经度约 106.24°E',apparentSolarTime:'约 03:11–04:11；04:30 中点约 03:41',bazi:natal,status:'真太阳时校正后整个给定时段均为寅时，四柱稳定'},model:{weights:Object.fromEntries(keys.map((k,i)=>[k,weights[i]])),profiles,tenGodVectors:vectors,hiddenStems:hidden,relations,calendarMethod:'北京时间按东八区 120°E，固原约 106.24°E；2002-12-12 方程时约 +6.4 分钟，合并经度差后真太阳时约比北京时间提前 48 分 38 秒。给定 04:00–05:00 对应约 03:11–04:11，均属寅时。示例起运约 2004-06-25，流年以立春为界。',ohLC:'open=前一年close，首年50；close=score；spread=1+min(3,0.08×逐条非基准规则的总分贡献绝对值之和)；high/low=开收盘包络±spread，限制0–100。影线仅为较弱的K线视觉提示。',limitations:'自定义传统文化模型，系数未经经验校准。未实现完整格局、调候、三合三会及刑局、合化、神煞体系；没有现实事件回测。不预测收入、婚期或寿命。'},chartData:generate(calendar,profile)};}
