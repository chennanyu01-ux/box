...
def connect(path):
    ...
    c.execute('CREATE TABLE IF NOT EXISTS items(name TEXT PRIMARY KEY, display TEXT, enabled INTEGER DEFAULT 1)')
...
def upsert_items(c,items):
    c.executemany('INSERT OR REPLACE INTO items(name,display,enabled) VALUES(?,?,1)',[(x['marketHashName'],x.get('name','')) for x in items]); c.commit()
